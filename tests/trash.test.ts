...
describe("content purge respects package_contents even when RESTRICT alone would not catch it", () => {
  ...
  /*
   * Nothing else in the codebase unlinks an upload — a purged product image,
   * avatar or receipt stays on disk today, and that is a known, separate
   * gap. Resources are the one path this session touched, so this is the one
   * path it verifies: the row disappearing must not be the only thing that
   * happens, or "delete" is a lie the disk keeps for as long as the VPS has
   * room to hold it.
   */
  it("deletes the file on disk along with the row", async () => {
    const tree = await seedContentTree();
    ...
    const absolute = path.join(storageRoot, resource.filePath!);
    await mkdir(path.dirname(absolute), { recursive: true });
    await writeFile(absolute, "not a real pdf, just needs to exist");
  ...
  it("purging a resource whose file is already gone from disk still succeeds", async () => {
    const tree = await seedContentTree();
    // seedContentTree points filePath at a UUID that was never written —
    // the ordinary case for every other test in this file, and exactly the
    // "restored from a backup between upload and delete" case the purge
    // path has to tolerate rather than fail on.
    await db
      .update(schema.resources)
      .set({ archivedAt: new Date() })
      .where(eq(schema.resources.id, tree.years.L1.resourceId));
