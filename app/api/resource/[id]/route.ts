...
export async function GET(
  ...
  const isInline =
    ...
    mime === "image/png" ||
    mime === "image/webp";
  // `request.url`, not `nextUrl`: the latter exists only on a real
  // `NextRequest`, and this handler is called directly with a plain
  // `Request` by the route tests.
  const forceDownload =
    new URL(request.url).searchParams.get("download") === "1";
  const disposition = resource.allowDownload || forceDownload || !isInline ? "attachment" : "inline";
  ...
  if (range) {
    ...
    return new Response(nodeToWebStream(openRange(file.absolute, start, end)), {
      status: 206,
      headers,
    });
  }
  ...
  return new Response(nodeToWebStream(openRange(file.absolute, 0, file.size - 1)), {
    status: 200,
    headers,
  });
}
