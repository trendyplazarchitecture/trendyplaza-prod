...
import { StudentNotices } from "@/components/portal/StudentNotices";
...
export default async function DashboardPage({ params }: Props) {
  ...
  const audience = user.state === "on_hold" ? "on_hold" : "students";
  ...
  const [
    ...
    getSiteSettings(),
    getActiveAnnouncementForUser(user.id, locale, audience),
    listNews(locale, { perPage: 1 }),
    listUpcomingEvents(locale, 1),
    listMyRegisteredEvents(user.id, locale),
  ]);

  const live = held.filter((e: { status: string }) => e.status === "active");
  ...
  return (
    <div className="w-full space-y-6">
      <DashboardHero
        ...
        lede={hasAccess ? t("dashboardLede") : t("dashboardLedeLocked")}
      />

      {/* Announcements, Registered Events & Updates */}
      <StudentNotices
        announcement={activeAnnouncement}
        latestNews={latestNews.rows[0] ?? null}
        latestEvent={upcomingEvents[0] ?? null}
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          {!hasAccess && latestRequest?.status !== "pending" && packages.length > 0 && (
            <section className="rounded-xl border border-rule bg-card p-5">
              ...
              <ReceiptForm
                packages={packages.map((p: { id: string; title: string; priceDzd: number; defaultDurationDays: number | null }) => ({
                  id: p.id,
                  title: p.title,
                  priceDzd: p.priceDzd,
