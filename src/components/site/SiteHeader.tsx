...
import { ArrowUpRight, ChevronDown, LayoutGrid, Menu, MonitorCog, BookOpen, ShoppingBag, User, X } from "lucide-react";
...
/**
 * Sections that exist and are reachable, but are not yet a paid,
 * fully-launched part of the site header's main row — a holding pen
 * rather than a hidden URL. Both pages handle their own access on the way
 * in (Software Hub redirects when not entitled; the Library shows a
 * blurred preview), so listing them here is just "this exists, look" —
 * the same posture `nav-items.ts`'s own doc comment describes for the
 * portal sidebar.
 */
const OTHER_NAV = [
  { href: "/software-hub", key: "nav.softwareHub", Icon: MonitorCog },
  { href: "/digital-library", key: "nav.digitalLibrary", Icon: BookOpen },
] as const;
        ...
        <div className="mx-auto flex h-16 w-full max-w-[1400px] items-center gap-4 px-4 sm:px-6 lg:px-12">
          ...
          <nav className="mx-auto hidden items-center gap-1 md:flex" aria-label="Main">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                className={cn(
                  "flex items-center gap-1 rounded-md px-3 py-2 text-sm transition-colors duration-150 outline-none",
                  OTHER_NAV.some((item) => pathname.startsWith(item.href))
                    ? "font-semibold text-foreground"
                    : "text-muted-foreground hover:text-foreground",
                )}
              >
                ...
                >
                  ...
                  >
                    ...
                    <motion.div
                      initial={shouldReduceMotion ? false : { opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.05 + NAV.length * 0.05, duration: 0.35, ease: EASE_OUT }}
