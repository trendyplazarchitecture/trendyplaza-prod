...
import { mediaSrcSet, testimonialImageUrl } from "@/lib/media";
          ...
          return (
            ...
            >
              {url && (
                <img
                  src={url}
                  srcSet={mediaSrcSet(url, [240, 320, 480])}
                  sizes="(min-width: 640px) 260px, 220px"
                  alt="Student testimonial review screenshot"
                  loading="lazy"
                  decoding="async"
...
export function Reviews({ items }: { items: Testimonial[] }) {
  ...
  return (
    <section className="overflow-hidden border-b border-border bg-paper py-20">
      ...
      {/* The columns are decorative screenshots with no per-item text, so a
          hidden list of them said nothing a screen reader could use — and
          downloaded every full-size image a second time to say it. */}
      <p className="sr-only">{t("imageAlt")}</p>
    </section>
  );
}
