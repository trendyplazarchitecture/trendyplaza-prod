...
export type ResourceRow = {
  ...
  titleAr: string | null;
  titleFr: string | null;
  madeByEn: string | null;
  madeByAr: string | null;
  madeByFr: string | null;
  source: string;
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <TriLingualField
              name="madeBy"
              label={t("madeByFieldLabel")}
              hint={t("madeByHint")}
              englishRequired={false}
              values={
                editing
                  ? { En: editing.madeByEn, Ar: editing.madeByAr, Fr: editing.madeByFr }
                  : undefined
              }
            />
            ...
            {source === "file" ? (
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  type="file"
                  multiple={!editing}
                  accept=".pdf,.jpg,.jpeg,.png,.webp,.dwg,.dxf,.pptx,.docx,.xlsx,.ppt,.doc,.xls"
                />
                <p className="text-xs text-muted-foreground">
                  {t("fileHint")}{" "}
                  {editing ? t("leaveEmptyToKeep") : t("fileHintMultiple")}
                </p>
              </div>
            ) : (
              <div className="space-y-1.5">
