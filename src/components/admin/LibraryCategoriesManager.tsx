...
import {
  ...
  restoreLibraryCategoryAction,
  saveLibraryCategoryAction,
  type ActionResult,
...
/**
 * The shelves the library is organised into, editable from here instead of a
 * developer changing an enum. Template: `ProductCategoriesManager.tsx` —
 * `library_categories` is shaped identically to `product_categories`.
 * `library_items.category_id` is `RESTRICT`, so a category still holding
 * items cannot be deleted — archive it instead, which keeps existing items
 * correctly labelled but stops it being offered again as a choice.
 */
export function LibraryCategoriesManager({ categories }: { categories: LibraryCategoryRow[] }) {
  const t = useTranslations("admin.libraryCategories");
  const tc = useTranslations("admin.common");
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<LibraryCategoryRow | "new" | null>(null);
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    act(() => saveLibraryCategoryAction(form));
  }
        ...
        <ul className="divide-y divide-border rounded-lg border border-border">
          {categories.map((c) =>
            editing !== "new" && current?.id === c.id ? (
              ...
              <li
                key={c.id}
                className={cn("flex items-center justify-between gap-3 px-3 py-2.5", c.archivedAt && "opacity-60")}
              >
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {c.archivedAt ? (
                    <Button
                      ...
                      disabled={isPending}
                      onClick={() => act(() => restoreLibraryCategoryAction({ categoryId: c.id }))}
                    >
                    ...
                    <>
                      ...
                      <Button
                        ...
                        disabled={isPending}
                        onClick={() => act(() => archiveLibraryCategoryAction({ categoryId: c.id }))}
                      >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (confirm(t("deleteConfirm", { name: c.labelEn }))) {
                        act(() => deleteLibraryCategoryAction({ categoryId: c.id }));
                      }
                    }}
                  >
        ...
        {editing === "new" ? (
          <form onSubmit={submit} className="space-y-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => setEditing(null)}>
                <X className="h-3.5 w-3.5" />
              </Button>
            </div>
            <div className="space-y-1">
              <Label htmlFor="new-lib-cat-key" className="text-xs">
                {t("keyLabel")}
              </Label>
              <Input
                id="new-lib-cat-key"
          ...
          <Button type="button" size="sm" variant="outline" className="gap-1.5" onClick={() => setEditing("new")}>
