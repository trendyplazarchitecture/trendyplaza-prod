...
export type ViewableResource = {
  ...
  title: string;
  description: string;
  /** Teacher credit, in the reading locale. Empty when nobody was credited. */
  madeBy: string;
...
function isInlinePreviewable(mime: string | null | undefined): boolean {
...
export function ResourceList({
  ...
  /** Which streaming route to read a `file` resource through. Library items (NextPhase/03-library) fork their own — see `/api/library-item/[id]`. */
  baseUrl?: string;
}) {
...
/**
 * Exported separately from `ResourceList` so a caller with its own grid or
 * carousel layout (the public library, `03-library`) can drive the same
 * full-screen reader without adopting `ResourceList`'s own list markup.
 */
export function ResourceViewerPanel({
  items,
  index,
  baseUrl,
  onIndex,
        ...
        >
          <header className="flex h-14 shrink-0 items-center gap-2 border-b border-rule px-3 sm:px-4">
            <div className="min-w-0 flex-1">
              ...
              {/*
                One subtitle line, not two: the header is 56px and a second
                row pushes the viewport down on a phone. The position in the
                module and who made the resource sit together, separated.
              */}
              {(items.length > 1 || resource.madeBy) && (
                <p className="figures truncate text-[11px] text-muted-foreground">
                  {items.length > 1 && `${index + 1} / ${items.length}`}
                  {items.length > 1 && resource.madeBy && " · "}
                  {resource.madeBy && t("madeBy", { name: resource.madeBy })}
                </p>
              )}
            </div>
            ...
            {resource.source === "file" && (resource.allowDownload || !isInlinePreviewable(resource.mimeType)) && (
              <a
                href={`${baseUrl}/${resource.id}?download=1`}
                download
          ...
          >
            <Embed resource={resource} width={width} baseUrl={baseUrl} />
...
function Embed({ resource, width, baseUrl }: { resource: ViewableResource; width: Width; baseUrl: string }) {
  const t = useTranslations("library");
  ...
  // File formats that cannot be rendered inline (AutoCAD .dwg/.dxf, Word .docx,
  // PowerPoint .pptx, Excel .xlsx, etc.) offer direct download.
  return <DirectDownloadStage resource={resource} baseUrl={baseUrl} />;
}
...
function getFormatInfo(mime: string | null): { label: string; badge: string } {
  if (!mime) return { label: "Document", badge: "FILE" };
  if (mime === "image/vnd.dwg" || mime === "application/acad") return { label: "AutoCAD Drawing", badge: "DWG" };
  if (mime === "image/vnd.dxf" || mime === "application/dxf") return { label: "AutoCAD Interchange", badge: "DXF" };
  if (mime.includes("presentationml") || mime.includes("powerpoint")) return { label: "PowerPoint Presentation", badge: "PPTX" };
  if (mime.includes("wordprocessingml") || mime.includes("msword")) return { label: "Word Document", badge: "DOCX" };
  if (mime.includes("spreadsheetml") || mime.includes("excel")) return { label: "Excel Spreadsheet", badge: "XLSX" };
  if (mime === "application/zip") return { label: "ZIP Archive", badge: "ZIP" };
  return { label: "Study Material", badge: "FILE" };
}
...
function DirectDownloadStage({ resource, baseUrl }: { resource: ViewableResource; baseUrl: string }) {
  const t = useTranslations("library");
  const src = `${baseUrl}/${resource.id}?download=1`;
  const info = getFormatInfo(resource.mimeType);
  const size = readableSize(resource.sizeBytes);

  return (
    <div className="flex h-full w-full items-center justify-center p-4 sm:p-8">
      <div className="flex w-full max-w-md flex-col items-center gap-5 rounded-2xl border border-border bg-card p-6 text-center shadow-lg sm:p-8">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 text-primary">
          <Download className="h-8 w-8" aria-hidden="true" />
        </div>

        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 rounded-full border border-border bg-paper px-3 py-1 text-xs font-semibold text-foreground">
        ...
        <a
          href={src}
          download
          className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary-press"
        >
