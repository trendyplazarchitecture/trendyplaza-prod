...
import { MAX_RESOURCE_BYTES, storeUpload, isInlinePreviewable } from "@/server/storage";
...
const resourceInput = z.object({
  ...
  titleAr: triOther,
  titleFr: triOther,
  // Optional in every language, English included: an attribution nobody gave
  // is a blank, not a validation error.
  madeByEn: triOther,
  madeByAr: triOther,
  madeByFr: triOther,
...
export async function saveResourceAction(formData: FormData): Promise<ActionResult> {
  ...
  const parsed = resourceInput.safeParse({
    ...
    madeByEn: (formData.get("madeByEn") as string) || null,
    madeByAr: (formData.get("madeByAr") as string) || null,
    madeByFr: (formData.get("madeByFr") as string) || null,
  ...
  // Editing an existing single resource
  if (data.id) {
    ...
    const values = {
      ...
      madeByEn: data.madeByEn ?? null,
      madeByAr: data.madeByAr ?? null,
      madeByFr: data.madeByFr ?? null,
      source: data.source,
    ...
    await logActivity({
      actorId: actor.id,
      action: "content.resource_updated",
      entity: "resource",
      entityId: data.id,
  ...
  // Multiple files selected at once in the dialog
  if (uploads.length > 1) {
    let created = 0;
    const errors: string[] = [];
    ...
    for (let i = 0; i < uploads.length; i++) {
      ...
      if (!stored.ok) {
        errors.push(
          `${upload.name} (${stored.error === "too_large" ? "over 200 MB" : "unsupported format"})`,
        );
        continue;
      }

      const titleEn = i === 0 && data.titleEn ? data.titleEn : titleFromFilename(upload.name);
      // Auto-allow direct download for non-previewable files (CAD, Office docs, etc.)
      const allowDownload = data.allowDownload || !isInlinePreviewable(stored.mime);

      await db.insert(resources).values({
        moduleId: data.moduleId,
        resourceTypeId: data.resourceTypeId,
        titleEn,
        titleAr: i === 0 ? data.titleAr : null,
        titleFr: i === 0 ? data.titleFr : null,
        // Unlike the title, the credit applies to every file in the batch: a
        // teacher dropping a semester of their own TDs is crediting all of
    ...
    if (created === 0) {
      return {
        ok: false,
        message: `Failed to upload files: ${errors.join(", ")}. Accepted formats: PDF, images, AutoCAD (.dwg, .dxf), Word (.docx), PowerPoint (.pptx), and Excel (.xlsx).`,
      };
    }
    ...
    return {
      ...
      message:
        errors.length === 0
          ? `${created} resources added successfully.`
          : `${created} added. ${errors.length} skipped (${errors.join(", ")}).`,
    };
  }
...
export async function bulkAddResourcesAction(formData: FormData): Promise<BulkResult> {
  ...
  for (const file of files) {
    const stored = await storeUpload(
      "resources",
      { buffer: Buffer.from(await file.arrayBuffer()), declaredName: file.name },
      { maxBytes: MAX_RESOURCE_BYTES, convertImages: false },
    );
    ...
    // Auto-allow direct download for non-previewable formats (AutoCAD, Office docs, etc.)
    const allowDownload = !isInlinePreviewable(stored.mime);
  ...
  if (created === 0) {
    return {
      ok: false,
      message:
        "None of those files were an accepted type. Use PDF, images, AutoCAD (.dwg, .dxf), Word (.docx), PowerPoint (.pptx), or Excel (.xlsx).",
    };
  }
