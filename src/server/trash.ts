...
const REGISTRY = {
  ...
  software_tool: {
    table: softwareTools,
    permission: "software.manage" as Permission,
    activityEntity: "software_tool",
    title: (r: typeof softwareTools.$inferSelect) => r.nameEn,
  },
  library_item: {
    table: libraryItems,
    permission: "library.manage" as Permission,
    activityEntity: "library_item",
    title: (r: typeof libraryItems.$inferSelect) => r.titleEn,
  },
...
async function deleteChildren(entity: TrashEntity, id: string, tx: Executor) {
  ...
  if (entity === "library_item") {
    // eslint-disable-next-line no-restricted-syntax -- join-table cleanup, not a delete of any tag itself, see the doc comment above.
...
export async function purgeFromTrash(
  ...
  // The row is gone; the file behind it is not, unless this cleans it up.
  // Nothing else in the codebase ever unlinks an upload, so a resource that
  // is purged without this stays on disk forever, counting against a VPS
  // with a few GB to share between the database and every course PDF.
  if (entity === "content_resource") {
    const resourceRow = row as unknown as typeof resources.$inferSelect;
    if (resourceRow.source === "file" && resourceRow.filePath) {
      await deleteStored(resourceRow.filePath);
    }
  }
  if (entity === "library_item") {
    const itemRow = row as unknown as typeof libraryItems.$inferSelect;
    if (itemRow.source === "file" && itemRow.filePath) {
      await deleteStored(itemRow.filePath);
    }
    if (itemRow.coverImagePath) {
      await deleteStored(itemRow.coverImagePath);
    }
  }
