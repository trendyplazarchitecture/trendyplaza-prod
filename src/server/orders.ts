...
export type CheckoutResult =
  | { ok: true; reference: string; orderId: string; totalDzd: number; notify: NotificationPayload | null }
  | {
      ok: false;
      error:
        | "phone_invalid"
...
export async function placeOrder(
  ...
  // A manually created order is one the admin just typed in themselves —
  // telling them it exists is not news. Only a customer-placed order builds
  // a notification for the browser to send. `result.ok` is always `true`
  // here -- the transaction above has no path that returns anything else --
  // so there is no failure branch to check before this.
  const notify = options.createdByAdminId
    ? null
    : await buildOrderNotification(result.reference, result.totalDzd, input, lines);

  return { ...result, notify };
}

/** Kept separate from `placeOrder` so building this never risks the commit above. */
async function buildOrderNotification(
  reference: string,
  totalDzd: number,
  input: z.infer<typeof checkoutInput>,
  lines: { product: { titleEn: string }; color: { nameEn: string } | null; quantity: number }[],
): Promise<NotificationPayload | null> {
  const [geo] = await db
    .select({ wilayaNameFr: wilayas.nameFr, communeNameFr: communes.nameFr })
    .from(communes)
    .innerJoin(wilayas, eq(wilayas.code, communes.wilayaCode))
    .where(eq(communes.id, input.communeId))
    .limit(1);

  const itemsSummary = lines
    .map((l) => `${l.quantity}x ${l.product.titleEn}${l.color ? ` (${l.color.nameEn})` : ""}`)
    .join(", ");

  return buildNotification({
    subject: `New order ${reference} — ${formatDzd(totalDzd)}`,
    ...
    fields: {
      ...
      Phone: input.phone,
      Total: formatDzd(totalDzd),
      Items: itemsSummary,
      Delivery: geo
        ? `${geo.communeNameFr}, ${geo.wilayaNameFr} — ${input.deliveryType === "home" ? "home delivery" : "desk pickup"}`
