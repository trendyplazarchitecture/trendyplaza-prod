...
> {
  try {
    ...
    // Not inside this function's own try/catch: the receipt is already
    // saved at this point, and a failure looking up who to credit in the
    // notification must never turn a successful submission into a reported
    // failure — buildReceiptNotification has its own, and returns null.
    const notify = await buildReceiptNotification(userId, input.packageId, input.amountClaimedDzd);
...
/** Kept separate from `submitAccessRequest` so this always resolves, even to `null`, rather than risk the outer try/catch above. */
async function buildReceiptNotification(
  userId: string,
  packageId: string,
  amountClaimedDzd: number | null,
  ...
  try {
    const [context] = await db
      .select({
        ...
        packageTitle: lmsPackages.titleEn,
      })
      .from(users)
      .innerJoin(lmsPackages, eq(lmsPackages.id, packageId))
    ...
    return buildNotification({
      subject: `New receipt to review — ${context.studentName}`,
      dashboardPath: "/admin/requests",
      replyTo: context.studentEmail,
      fields: {
        Student: context.studentName,
        Email: context.studentEmail,
        Package: context.packageTitle,
        "Amount claimed": amountClaimedDzd !== null ? formatDzd(amountClaimedDzd) : "not given",
      },
    });
  } catch (error) {
    console.error("notify: failed to prepare new-receipt notification", error);
    return null;
  }
}
