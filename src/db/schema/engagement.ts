...
import { announcementAudience, contactStatus, postAudience, postKind } from "./enums";
...
export const resourceViews = pgTable(
  ...
  (t) => [
    index("resource_views_user_resource_idx").on(t.userId, t.resourceId),
...
/**
 * A study plan: an ordered sequence of resources an admin recommends for a
 * topic (e.g. "Rhino for Beginners"). Students check them off as they go.
 */
...
/**
 * Notice board & engagement posts: Announcements, Events & Workshops, News.
 */
export const posts = pgTable("posts", {
  id: uuid().primaryKey().defaultRandom(),
  kind: postKind().notNull().default("announcement"),
  slug: text().unique(),
  titleEn: text().notNull(),
  titleFr: text(),
  titleAr: text(),
  ...
  audience: postAudience().notNull().default("all"),
  startsAt: tsz("starts_at"),
  endsAt: tsz("ends_at"),
  locationEn: text(),
  locationFr: text(),
...
/** Presence of a row is what makes the popup/banner show once and not again. */
export const postViews = pgTable(
  "post_views",
  {
    postId: uuid()
      .notNull()
      .references(() => posts.id, RESTRICT),
    userId: text()
      .notNull()
      .references(() => users.id, RESTRICT),
    seenAt: createdAt(),
  ...
  (t) => [
    ...
    index("post_views_user_idx").on(t.userId),
  ],
);

/**
 * Native Event Registrations / RSVPs.
 */
export const eventRegistrations = pgTable(
  "event_registrations",
  {
    ...
    postId: uuid()
      .notNull()
      .references(() => posts.id, RESTRICT),
    userId: text().references(() => users.id, RESTRICT),
    name: text().notNull(),
    phone: text().notNull(),
    email: text().notNull(),
    university: text(),
  ...
  (t) => [
    index("event_registrations_post_idx").on(t.postId),
